function I = preprocessor_03_R_V_H_P(imagem)
    ...
    for y = 1:size(imagemCropDedos,1)
        if imagemCropDedos(y, size(imagemCropDedos,2), :) > 45
            yMaoSuperior = y;
            break
        end
    end
    for y = size(imagemCropDedos,1):-1:1
        if imagemCropDedos(y, size(imagemCropDedos,2)-150, :) > 45
            yMaoInferior = y;
            break
        end
    end
    ...
    centroid = [fix(0.57*size(imagemCrop,1)), fix(0.65*size(imagemCrop,2))];
    d1 = 80;
    d2 = 100;
