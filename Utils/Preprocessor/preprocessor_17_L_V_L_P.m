function I = preprocessor_17_L_V_L_P(imagem)
    for y = 1:size(imagem,1)
        if imagem(y, size(imagem,2), 1) > 45 || imagem(y, size(imagem,2), 2) > 45 || imagem(y, size(imagem,2), 3) > 45
            yMaoSuperior = y;
            break
        end
    end
...
%     imwrite(ROI, strcat(path,'_mao.tif'));
