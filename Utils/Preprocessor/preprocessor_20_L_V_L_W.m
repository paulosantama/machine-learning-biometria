function I = preprocessor_20_L_V_L_W(imagem)
    for y = 1:size(imagem,1)
        if imagem(y, size(imagem,2)-200, 1) > 45 || imagem(y, size(imagem,2)-200, 2) > 45 || imagem(y, size(imagem,2)-200, 3) > 45
            yMaoSuperior = y;
            break
        end
    end
    ...
    centroid = [fix(0.50*size(imagemCrop,1)), fix(0.62*size(imagemCrop,2))];
    d1 = 120;
    d2 = 350;
